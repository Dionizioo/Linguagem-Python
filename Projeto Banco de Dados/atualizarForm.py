...
class AtualizarForm:
    def __init__(self, id, rnome, rtelefone, remail, rlugar, rsexo, conn, master=None):
    ...
    def confirm(self):
        ...
        vlugar = self.pais.get()
        vsexo = self.sexo.get()
        
        if(len(vnome)==0):
            vnome=self.rnome
        if(len(vtelefone)==0):
            vtelefone=self.rtelefone
        if(len(vemail)==0):
            vemail=self.remail
        if(len(vlugar)==0):
            vlugar=self.rlugar
        ...
        vsql="UPDATE tb_contatos SET T_NOMECONTATO='"+vnome+"',T_TELEFONECONTATO='"+vtelefone+"',T_EMAILCONTATO='"+vemail+"' WHERE N_IDCONTATO="+self.vid
        vsql1="UPDATE tb_lugar SET T_PAIS='"+vlugar+"' WHERE N_IDCONTATO="+self.vid
        vsql2="UPDATE tb_sexo SET T_SEXO='"+vsexo+"' WHERE N_IDCONTATO="+self.vid
